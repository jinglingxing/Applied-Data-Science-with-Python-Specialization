...
def answer_three():
    Top15 = answer_one()
    AVG = Top15.iloc[:, 10:].mean(axis = 1, skipna = True)
...
#in regular expression, .* will be greedy and try to match as much as possible
    ...
    return country_max_energy['% Renewable']
####################################     Question 7     #################################### 
...
def answer_seven():
    ...
    return country_max_ratio['ratio']
...
#Use only the last 10 years (2006-2015) of GDP data and only the top 15 countries by Scimagojr 'Rank' (Rank 1 through 15).
    ...
    answer9= df_9['Citations per document'].corr(df_9['Energy Supply per Capita'])
    return answer9
...
def answer_eleven():   
    ...
    df_energy_cont = pd.merge(df_cont_2,TOP15, how ='left', left_on = 'Country', right_on = 'Country')
